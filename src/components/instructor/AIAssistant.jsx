...
export default function AIAssistant() {
  ...
  const [conversation, setConversation] = useState([
    {
      role: "assistant",
      content: "¡Hola! Soy tu asistente virtual del STIMI. ¿En qué puedo ayudarte hoy? Puedo responder preguntas sobre cómo completar informes, fechas de entrega, documentos requeridos y más."
    }
  ]);
  const suggestions = [
  ...
  return <div className="p-8">
    ...
    /* Messages */
          ...
          <div className="flex-1 p-6 overflow-y-auto space-y-4">
            {conversation.map((msg, index) => <div
    key={index}
    className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}
  >
                <div
    className={`max-w-[80%] p-4 rounded-2xl ${msg.role === "user" ? "bg-[#39A900] text-white" : "bg-gray-100 text-gray-800"}`}
  >
                  {msg.role === "assistant" && <div className="flex items-center gap-2 mb-2">
                      <Bot size={16} />
                      <span className="text-xs">Asistente STIMI</span>
                    </div>}
