...
export default function Sidebar({
  ...
  return <div className="w-72 bg-white border-r border-gray-100 h-screen flex flex-col shadow-sm">
      {
    /* Logo */
  }
      <div className="p-6 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="bg-gradient-to-r from-[#39A900] to-green-600 bg-clip-text text-transparent">STIMI</h2>
    ...
    return <Button
      ...
      className={`w-full justify-start gap-3 h-12 rounded-xl transition-all duration-200 ${item.id === "trash" && isActive ? "bg-red-50 text-red-600 hover:bg-red-100 shadow-sm" : item.id === "trash" ? "text-gray-500 hover:bg-red-50 hover:text-red-600" : isActive ? "bg-gradient-to-r from-green-50 to-emerald-50 text-[#39A900] hover:from-green-100 hover:to-emerald-100 shadow-sm" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}
    >
                <Icon size={20} strokeWidth={isActive ? 2.5 : 2} />
                <span className="flex-1 text-left">{item.label}</span>
                {item.badge && item.badge > 0 && <Badge className="bg-gradient-to-r from-[#39A900] to-green-600 text-white border-0 shadow-md">
                    {item.badge}
                  </Badge>}
              </Button>;
  })}
