...
import { FileText, Search, CheckCircle, XCircle, Clock, Eye, FileSignature, Filter } from 'lucide-react';
...
export default function ReportManagement() {
  ...
  const filteredReports = reports.filter(report =>
    report.instructor.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const ReportCard = ({ report }) => (
    <div className="p-5 border-2 rounded-2xl transition-all hover:shadow-lg bg-gradient-to-r from-gray-50/50 to-blue-50/50 border-gray-200 hover:border-blue-300">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className={`px-4 py-2 rounded-xl text-xs font-medium shadow-sm ${
            report.status === 'approved' ? 'bg-gradient-to-r from-green-100 to-emerald-100 text-green-700' :
            report.status === 'correcting' ? 'bg-gradient-to-r from-orange-100 to-amber-100 text-orange-700' :
            'bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-700'
          }`}>
            {report.status === 'approved' ? 'Aprobado' :
             report.status === 'correcting' ? 'A Corregir' :
             'Pendiente'}
          </div>
                ...
                <div className="border-t-2 pt-4">
                  ...
                  <div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-2xl p-8 text-center border-2 border-dashed border-gray-300">
                    ...
                    <p className="text-gray-700">
                      {report.type === 'GC' 
                        ? 'Informe de Gestión Contractual con 17 obligaciones documentadas'
                        : 'Informe de Gestión Financiera con todos los comprobantes'
                      }
                    </p>
                  </div>
                </div>
      ...
      <Tabs defaultValue="pending" className="space-y-6">
        ...
        <TabsContent value="correcting" className="space-y-3">
          {correctingReports.length === 0 ? (
            ...
            correctingReports.map(report => <ReportCard key={report.id} report={report} />)
          )}
        </TabsContent>
      </Tabs>
    </div>
